...
export function renderPage(canvas,page,index,total,date,image,crop,options={}){canvas.width=W;canvas.height=H;const ctx=canvas.getContext('2d');const memo=options.memo?.trim()||'';const align=options.align||'left';const bodyFont=options.bodyFont||'Diary';const scaleX=options.scaleX||1;const bodySpacing=options.letterSpacing||0;const condensed=options.condensed||false;ctx.fillStyle='#fffdf4';ctx.fillRect(0,0,W,H);
  ctx.strokeStyle='#d9dfd4';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(M,216);ctx.lineTo(W-M,216);ctx.stroke();
  const dateStyle={scaleX,letterSpacing:condensed?-46*.03:0};ctx.fillStyle='#354b41';ctx.font=`700 46px ${bodyFont},${EMOJI_STACK}`;const dateWidth=drawStyled(ctx,dateLabel(date),M,166,dateStyle);
  if(memo){const memoStyle={scaleX,letterSpacing:condensed?-26*.03:0};ctx.fillStyle='#8b9585';ctx.font=`26px ${bodyFont},${EMOJI_STACK}`;drawStyled(ctx,fitText(ctx,memo,PW-dateWidth-38,memoStyle),W-M,166,{...memoStyle,align:'right'});}
